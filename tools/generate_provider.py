#!/usr/bin/env python3
"""Generate internal/provider/datasources.go and resources.go.

Replaces the former generate_datasources.py + generate_resources.py. Both
read generator_config.yml (and resources.go also reads the prepared spec) to
emit the Go wrappers around the tfplugingen-framework generated schemas, so
they are merged here to share helpers and avoid drift.

Usage: generate_provider.py <generator_config.yml> <prepared_spec.json>
"""
...
# --- data sources (was generate_datasources.py) ------------------------------

def emit_datasources(config: dict) -> str:
...
\t}}
    ...
    return f"""// Code generated by tools/generate_provider.py DO NOT EDIT.
...
# --- resources (was generate_resources.py) -----------------------------------
...
def emit_resources(config: dict, spec: dict) -> str:
    ...
    lines = [
        "// Code generated by tools/generate_provider.py; DO NOT EDIT.",
        "",
        "package provider",
        "",
        "import (",
...
def main() -> None:
    if len(sys.argv) < 3:
        print(f"usage: {sys.argv[0]} <generator_config.yml> <prepared_spec.json>", file=sys.stderr)
        sys.exit(1)

    config = json.loads(Path(sys.argv[1]).read_text())
    spec = json.loads(Path(sys.argv[2]).read_text())

    ds_path = Path("internal/provider/datasources.go")
    res_path = Path("internal/provider/resources.go")

    ds_path.write_text(emit_datasources(config))
    print(f"wrote {ds_path}")

    res_path.write_text(emit_resources(config, spec))
