...
def main() -> None:
    ...
    # Determine which configured resources have a generated package. Hand-written
    # resources are excluded by design; missing generated packages are warned.
    eligible_resources = set(resources.keys()) - HAND_WRITTEN_RESOURCES
    for name in sorted(eligible_resources):
        if not Path(f"internal/provider/generated/resource_{name}").exists():
            print(
                f"warning: skipping resource {name!r}; generated package resource_{name} is missing",
                file=sys.stderr,
            )
    ...
    for name, cfg in sorted(resources.items()):
        if name not in eligible_resources:
            continue
        if not Path(f"internal/provider/generated/resource_{name}").exists():
            continue
        create = cfg["create"]
        ...
        # genericResource hard-codes Post/Get/Delete at runtime, so only those
        # HTTP verbs are safe to emit. Update is dispatched to Put or Patch.
        if create["method"] != "post":
            raise ValueError(f"resource {name!r}: create method must be 'post', got {create['method']!r}")
        if read["method"] != "get":
            raise ValueError(f"resource {name!r}: read method must be 'get', got {read['method']!r}")
        if delete["method"] != "delete":
            raise ValueError(f"resource {name!r}: delete method must be 'delete', got {delete['method']!r}")
        if update["method"] not in ("put", "patch"):
            raise ValueError(f"resource {name!r}: update method must be 'put' or 'patch', got {update['method']!r}")
